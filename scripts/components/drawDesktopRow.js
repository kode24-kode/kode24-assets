...
export const initDesktopRow = (desktopRowNode, articlesSectionsData) => {
...
function drawDesktopRow(articlesData) {
  ...
  return `
    <div class="row desktop-row ${articlesData.style}">
      ...
      <div class="${articlesData.layout ? articlesData.layout : "multiple"}">
      ${articlesData.articles
        .map((article, index) =>
          articlesData.style === "commercial"
            ? drawDesktopAd(article)
            : drawDesktopArticle(article)
        )
        .join("")}
      </div>
    </div>
...
const drawDesktopArticle = (article) => {
