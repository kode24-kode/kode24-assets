...
function fixSanityUrl(url: string) {
  ...
  try {
    ...
    // Legg til format=webp hvis den ikke finnes
    if (!u.searchParams.has("format")) {
      u.searchParams.append("format", "webp");
    }

    // Returner ferdig kode24-cache-versjon
...
export default function CompanyPartnersTile({
  ...
  return (
    ...
    >
        ...
        <ul className="logo-list ">
          {shuffledCompanyPartners.map(
            (companyPartner: CompanyPartner, key: number) => (
              <li key={key}>
                <a href={"https://partner.kode24.no/" + companyPartner.slug}>
                  <img
                    className="dark"
                    src={getImageCacheUrl(
                      fixSanityUrl(companyPartner.darkLogo)
                    )}
                  ...
                  <img
                    className="light"
                    src={getImageCacheUrl(
                      fixSanityUrl(companyPartner.lightLogo)
                    )}
                    alt={"partner logo " + companyPartner.slug}
                    loading="lazy"
                  />
      ...
      <div className="partner-slider constrained">
        <Carousel
          swipeable={true}
          draggable={false}
          showDots={false}
                ...
                <div key={key}>
                  ...
                  >
                    <div className="partner-slider-image">
                      <img
                        src={
                          companyPartnersBannersLookup[partner.banner] ||
                    ...
                    <div className="partner-slider-title">
                      <h3
                        className={`partner-slider-title-slogan ${
                          partner.title.split("").length > 60 ? "small" : ""
                        }`}
                      >
                        {partner.title}
                      </h3>
