...
export default function ArticlesRow({
  ...
  const hottestArticle = findHottestArticle(DesktopRowData, hotnessThreshold);
  return (
    <div
      ...
      ${listView ? "list-view" : ""}
        `}
    >
      <div className={`heading ${DesktopRowData.title ? "" : "no-title"}`}>
      ...
      <div className={DesktopRowData.layout}>
        {DesktopRowData.articles.map((article: Article, key: number) => (
          <ArticleTile
            Article={article}
            key={key}
            isHot={
              hottestArticle && hottestArticle === article.id ? true : false
            }
          />
        ))}
      </div>
