                ...
                for line_num, line in enumerate(lines, 1):
                    ...
                    for component_name in component_names:
                        ...
                        import_patterns = [
                            ...
                            rf'import\s+.*\s+from\s+["\']\.\./components/{component_name}["\']',
                            rf'import\s+.*\s+from\s+["\']\.\./\.\./components/{component_name}["\']',
                            rf'import\s+.*\s+from\s+["\']src/components/{component_name}["\']',
                            rf'import\s+.*\s+from\s+["\']\./\.\./components/{component_name}["\']',
                        ]
                        ...
                        # Check for JSX usage (more comprehensive)
                        jsx_patterns = [
                            rf'<{component_name}\b',
                            rf'<{component_name}[A-Z]',
                            rf'<{component_name}[^a-zA-Z]',
                            rf'<{component_name}\s',
                            rf'<{component_name}>',
                            rf'<{component_name}/>',
                        ]
...
def analyze_component_usage():
    ...
    # Find unused components (both import and usage)
    used_components = set(import_data.keys()) | set(usage_data.keys())
    ...
    with open('component_usage_detailed_improved.txt', 'w', encoding='utf-8') as f:
        f.write("Component Usage Analysis (Improved)\n")
        f.write("=" * 50 + "\n\n")
    ...
    with open('unused_components_improved.txt', 'w', encoding='utf-8') as f:
    ...
    with open('component_usage_categorized_improved.txt', 'w', encoding='utf-8') as f:
        f.write("Component usage by category (improved):\n")
        f.write("=" * 40 + "\n\n")
    ...
    print(f"\n💾 Results saved to:")
    print(f"   - component_usage_detailed_improved.txt")
    print(f"   - component_usage_categorized_improved.txt")
    print(f"   - unused_components_improved.txt")

    # Show components with usage but no imports (might be self-referencing)
    self_referencing = []
    for component_name in component_names:
        if component_name in usage_data and component_name not in import_data:
            self_referencing.append(component_name)

    if self_referencing:
        print(f"\n🔄 Components with usage but no imports (self-referencing?):")
        for component_name in sorted(self_referencing):
            usage_count = len(usage_data.get(component_name, []))
            print(f"   • {component_name} ({usage_count} usages)")
